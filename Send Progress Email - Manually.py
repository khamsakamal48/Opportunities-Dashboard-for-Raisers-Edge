...
import requests, os, json, glob, csv, psycopg2, sys, smtplib, ssl, imaplib, time, datetime, logging, locale, xlsxwriter, pretty_html_table, shutil, urllib.parse, base64
...
def identify_current_quarter():
    global current_quarter, current_quarter_end_date, previous_quarter_start_date, previous_quarter_end_date, pp_previous_quarter_end_date
    ...
    if current_month >= 4 and current_month <= 6:
        ...
        previous_quarter_start_date = datetime.strptime(f"01-01-{current_year}", "%d-%m-%Y").date()
    ...
    # Current month between Jul and Sep
        ...
        previous_quarter_start_date = datetime.strptime(f"01-04-{current_year}", "%d-%m-%Y").date()
...
def create_empty_dataframe():
    ...
    data = [['', '', '', '', '', '']]
    empty_dataframe_with_id = pd.DataFrame(data, columns = ['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount'])
...
def get_pipeline(stage, type):
    global previous_quarter_stage_total, html_output_summary_table, html_output_detailed_table
    
    previous_quarter_stage_total = ""
    html_output_summary_table = ""
    html_output_detailed_table = ""
    ...
    ## Previous Quarter
    get_quarterwise_data(previous_quarter_dataframe, "Previous", type, stage, workbook)
    previous_quarter_stage_dataframe = quarter_dataframe
    previous_quarter_stage_total = quarter_dataframe_total
    
    ## Current Quarter
    get_quarterwise_data(current_quarter_dataframe, "Current", type, stage, workbook)
    current_quarter_stage_dataframe = quarter_dataframe
    current_quarter_stage_total = quarter_dataframe_total
    current_quarter_stage_count = len(current_quarter_stage_dataframe.index)
    
    ## Working to get newly added
    newly_added_opportunity_id = list(set(current_quarter_stage_dataframe['Opportunity_ID']) - set(previous_quarter_stage_dataframe['Opportunity_ID']) - set(previous_quarter_dataframe['Opportunity_ID']))
    get_stagewise_data(newly_added_opportunity_id, current_quarter_stage_dataframe, "Current", type, stage, "Newly added", workbook)
    newly_added_stage = new_dataframe
    newly_added_stage_count = new_dataframe_count
    newly_added_stage_total = new_dataframe_total
    
    ## Working to get rejected
    current_quarter_rejected_dataframe = current_quarter_dataframe.query(f'Type == "{type}" and Status == "Rejected"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    rejected_opportunity_id = list(current_quarter_rejected_dataframe['Opportunity_ID'])
    get_stagewise_data(rejected_opportunity_id, previous_quarter_stage_dataframe, "Current", type, "Rejected", stage, workbook)
    rejected_stage_dataframe = new_dataframe
    rejected_stage_count = new_dataframe_count
    rejected_stage_total = new_dataframe_total
    
    ## Working to get moved
    prospect_dataframe = current_quarter_dataframe.query(f'Type == "{type}" and Status == "Prospect"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    cultivation_dataframe = current_quarter_dataframe.query(f'Type == "{type}" and Status == "Cultivation"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    solicitation_dataframe = current_quarter_dataframe.query(f'Type == "{type}" and Status == "Solicitation"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    committed_dataframe = current_quarter_dataframe.query(f'Type == "{type}" and Status == "Committed"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    converted_dataframe = current_quarter_dataframe.query(f'Type == "{type}" and Status == "Converted"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    
    previous_prospect_dataframe = previous_quarter_dataframe.query(f'Type == "{type}" and Status == "Prospect"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    previous_cultivation_dataframe = previous_quarter_dataframe.query(f'Type == "{type}" and Status == "Cultivation"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    previous_solicitation_dataframe = previous_quarter_dataframe.query(f'Type == "{type}" and Status == "Solicitation"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    previous_committed_dataframe = previous_quarter_dataframe.query(f'Type == "{type}" and Status == "Committed"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    previous_converted_dataframe = previous_quarter_dataframe.query(f'Type == "{type}" and Status == "Converted"').filter(['Constituent_ID', 'Opportunity_ID', 'Opportunity_Name', 'Ask Amount', 'Expected Amount', 'Funded Amount']).drop_duplicates()
    ...
    if stage != "Prospect":
          ...
          create_empty_dataframe()
          previous_stage_dataframe = empty_dataframe_with_id
    ...
    ## Working to get moved to previous stages
    moved_to_stage_dataframe = ""
    moved_to_stage_dataframe_count = ""
    ...
    # Prepare HTML for Corporate Prospect Summary
    prepare_summary_table(previous_quarter_stage_total, current_quarter_stage_total)
    html_output_summary_table = html_output
    print(html_output_summary_table)
    
    prepare_detailed_table(newly_added_stage_total, newly_added_stage_count, moved_stage_dataframe_total, moved_stage_dataframe_count,
...
def prepare_detailed_table(newly_added, newly_added_count, moved, moved_count, rejected, rejected_count, carried_forward, carried_forward_count, moved_back, moved_back_count, withdrawn_to, withdrawn_to_count, stage):
        ...
        TEMPLATE = """
                ...
                "data": {
                    ...
                    },
                    {
                        ...
                        "borderWidth": 1,
                        "data": [
                        {{previous_quarter_prospect_total}},
                        {{carried_forward}}
                        ],
                        "fill": false,
                        "spanGaps": false,
    ...
    chart = Environment().from_string(TEMPLATE).render(
        stage = stage,
        previous_quarter_prospect_total = str(previous_quarter_stage_total).replace("₹", "").replace(" Cr.", ""),
...
def send_email():    
    subject = "Opportunity Pipeline Progress | Raisers Edge"
    ...
    TEMPLATE = """
                ...
                <head> 
                  ...
                  <title>Opportunity Pipeline Progress | Raisers Edge</title><!--[if (mso 16)]>
                    <style type="text/css">
                ...
                <body style="width:100%;font-family:arial, 'helvetica neue', helvetica, sans-serif;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;padding:0;Margin:0"><span style="display:none !important;font-size:0px;line-height:0;color:#ffffff;visibility:hidden;opacity:0;height:0;width:0;mso-hide:all">Dear Team, Please find the below progress report of the Opportunities as recorded in Raisers Edge.&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;</span> 
                                ...
                                <tr> 
                                  ...
                                  <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                    ...
                                    <tr> 
                                      ...
                                      {{corporate_html_output_prospect_summary_table}} 
                                      </td> 
                                    </tr> 
                                    <tr> 
                            ...
                            <tr> 
                              ...
                              <table cellpadding="0" cellspacing="0" width="100%" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                <tr> 
                                  ...
                                  <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                    <tr> 
                                      <td align="center" style="padding:20px;Margin:0"><h1 style="Margin:0;line-height:36px;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;font-size:30px;font-style:normal;font-weight:normal;color:#000000;text-align:center"><strong>Major Donor</strong></h1></td> 
                                    </tr> 
                                    <tr> 
                                      <td align="center" style="padding:0;Margin:0;padding-left:20px;padding-right:20px;font-size:0"> 
                                      <table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                    ...
                                    <tr> 
                                      ...
                                      <tr> 
                                        <td align="center" style="padding:0;Margin:0;padding-bottom:20px;padding-left:20px;padding-right:20px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px"><br></p> 
                                          {{major_donor_html_output_prospect_summary_table}}
                                        </td> 
                                      </tr> 
                                        <tr>
                                        ...
                                        <tr> 
                                          <td align="center" style="padding:0;Margin:0;padding-bottom:20px;padding-left:20px;padding-right:20px"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px"><br></p> 
                                            {{major_donor_html_output_prospect_detailed_table}}
                                          </td> 
                                        </tr> 
                                    <tr> 
                                      <td align="center" style="padding:0;Margin:0;padding-top:20px;padding-bottom:0px;padding-left:20px;padding-right:20px;"><h2 style="Margin:0;line-height:29px;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;font-size:24px;font-style:normal;font-weight:normal;color:#000000;text-align:center"><strong>Cultivation</strong></h2></td> 
