...
def get_constituent_data():
    ...
    constituent_dataframe = pd.DataFrame(constituent_data, columns = ['value.constituent_id', 'constituent_name', 'type'])
    
def get_campaign_data():
    
    global campaign_dataframe
    
    extract_sql = """
    SELECT * from campaign_list
    """
    cur.execute(extract_sql)
    campaign_data = list(cur.fetchall())
    
    # Converting to Panda's Dataframe
    print("Converting to Panda's Dataframe")
    campaign_dataframe = pd.DataFrame(campaign_data, columns = ['campaign_id', 'campaign_name'])
    
def prepare_html_table(dataframe):
    global html_output
    
    data = pd.DataFrame(dataframe)
    html_output = (build_table(data, 'blue_dark', font_family='Open Sans, Helvetica, Arial, sans-serif', even_color='black', padding='10px', width='900px', font_size='16px', text_align='center')).replace("background-color: #D9E1F2;font-family: Open Sans", "background-color: #D9E1F2; color: black;font-family: Open Sans").replace('Total', '<b>Total</b>').replace(total_donation, f'<b>{total_donation}</b>')
    
    print(html_output)
    
def identify_current_quarter():
    global current_quarter, current_quarter_start_date, current_quarter_end_date, previous_quarter_end_date, pp_previous_quarter_end_date, financial_year
    ...
    if current_month >= 4 and current_month <= 6:
        current_quarter = "Q1"
        current_quarter_start_date = datetime.strptime(f"01-04-{current_year}", "%d-%m-%Y").date()
    ...
    # Current month between Jul and Sep
        ...
        current_quarter = "Q2"
        current_quarter_start_date = datetime.strptime(f"01-07-{current_year}", "%d-%m-%Y").date()
    ...
    # Current month between Oct and Dec    
        ...
        current_quarter = "Q3"
        current_quarter_start_date = datetime.strptime(f"01-10-{current_year}", "%d-%m-%Y").date()
    ...
    # Current month between Jan and Mar    
        ...
        current_quarter = "Q4"
        current_quarter_start_date = datetime.strptime(f"01-01-{current_year}", "%d-%m-%Y").date()
    ...
    print(f"Current Quarter Start date: {current_quarter_start_date}")
...
def create_excel_file():
    ...
    global donations_workbook
    
    # Create a Pandas Excel writer using XlsxWriter as the engine.
    print("Creating Excel file for Donations")
    donations_workbook = pd.ExcelWriter(f'Donations_in_{current_quarter}_{financial_year}.xlsx', engine='xlsxwriter', engine_kwargs={'options':{'strings_to_urls': False}})
    
def save_excel_file():
    
    print("Saving Excel file for Donations")
    donations_workbook.save()
        
def get_donation(start_date):
    
    url = f"https://api.sky.blackbaud.com/gift/v1/gifts?gift_type=Donation&start_gift_date={start_date}&gift_type=MatchingGiftPayment&gift_type=PledgePayment&gift_type=RecurringGiftPayment"
...
def add_to_dataframe(i):
    ...
    if i == 1:
        ...
        dataframe_new = pd.json_normalize(value, record_path=['value', 'gift_splits'], meta=[['value', 'id'], ['value', 'amount', 'value'], ['value', 'constituent_id'], ['value', 'date'], ['value', 'lookup_id']])
        dataframe.append(dataframe_new)
        
def merge_dataframe(d1, d2, lookup_column):
    ...
    pprint(merged_dataframe)
    
def filter_dataframe(df, column, value):
    
    global filtered_dataframe
    
    filtered_dataframe = df.query(f'{column} == "{value}"')
        
    pprint(filtered_dataframe)
    
def add_formula_to_columns(dataframe):
    
    global dataframe_excel, excel_columns, dataframe_for_summary, excel_columns_for_summary
    
    excel_columns = ['Date', 'Name', 'Campaign', 'Amount']
    excel_columns_for_summary = ['Name', 'Amount']
    ...
    if dataframe.empty:
        ...
        dataframe_copy.columns = dataframe_copy.columns.str.replace('.', '', regex=False)
        
        # Adding Hyperlink to constituent name
        dataframe_copy['constituent_name'] = dataframe_copy.apply(lambda row: f'=HYPERLINK(CONCATENATE("https://host.nxt.blackbaud.com/constituent/records/",{row.valueconstituent_id},"?svcid=renxt&envid=p-dzY8gGigKUidokeljxaQiA"),"{row.constituent_name}")', axis=1)
        
        # Removing T00:00:00 from dates
        dataframe_copy['valuedate'] = dataframe_copy['valuedate'].str.replace('T00:00:00', '', regex=False)
        
        # Adding Hyperlink to Date
        dataframe_copy['valuedate'] = dataframe_copy.apply(lambda row: f'=HYPERLINK(CONCATENATE("https://host.nxt.blackbaud.com/gift/page/",{row.valueid},"?envid=p-dzY8gGigKUidokeljxaQiA&svcid=renxt"),"{row.valuedate}")', axis=1)
        
        # Replacing ',' with '-'
        dataframe_copy['campaign_name'] = dataframe_copy['campaign_name'].str.replace(' , ', ' - ', regex=False)
        
        # Adding Hyperlink to Campaign
        dataframe_copy['campaign_name'] = dataframe_copy.apply(lambda row: f'=HYPERLINK(CONCATENATE("https://host.nxt.blackbaud.com/cfa/campaigns/",{row.campaign_id},"?envid=p-dzY8gGigKUidokeljxaQiA&svcid=renxt"),"{row.campaign_name}")', axis=1)
        
        dataframe_excel = dataframe_copy.filter(['valuedate', 'constituent_name', 'campaign_name', 'amountvalue'])
        
        # Dataframe for summary
        dataframe_for_summary = dataframe.filter(['constituent_name', 'amount.value'])
        
        # Change Column names
        dataframe_excel.columns = excel_columns
        dataframe_for_summary.columns = excel_columns_for_summary
        
        pprint(dataframe_excel)
        
def create_empty_dataframe():
    global empty_dataframe, empty_dataframe_for_summary
    
    data = [['', '', '']]
    empty_dataframe = pd.DataFrame(data, excel_columns)
    
    data = [['', '']]
    empty_dataframe = pd.DataFrame(data, excel_columns_for_summary)
...
def write_to_excel(dataframe, workbook, worksheet):
    ...
    date_format = workbook_formatted.add_format(
        {
    ...
    worksheet_formatted.set_column('A:A', 15, date_format)
    worksheet_formatted.set_column('B:B', 60, hyperlink)
    worksheet_formatted.set_column('C:C', 100, hyperlink)
    worksheet_formatted.set_column('D:D', 30, money)
...
def summarize_dataframe(dataframe, type):
    
    global top_donor_dataframe, total_donation
    
    summarized_dataframe = dataframe.groupby('Name', as_index=False).sum('Amount')
    
    pprint(summarized_dataframe)
    
    # # Get donation above 50 Lakh
    # top_donor_dataframe = summarized_dataframe[summarized_dataframe['Amount'] >= 5000000].dropna().sort_values(by=['Amount'], ascending=False)
    
    top_donor_dataframe = []
    
    # Get top 9 Donors
    top_donor_dataframe = summarized_dataframe.nlargest(9, ['Amount'])
    
    # Get other donation below 50 Lakh
    other_donation = summarized_dataframe['Amount'].sum() - top_donor_dataframe['Amount'].sum()
    
    # Add an Other and Total Row
    total_donation = locale.currency(top_donor_dataframe['Amount'].sum(), grouping=True)
    
    other_donation_list = [['Others', other_donation], ['Total', top_donor_dataframe['Amount'].sum()]]
    total_donation_list = pd.DataFrame(other_donation_list, columns=['Name', 'Amount'])
    
    # Concat rows
    top_donor_dataframe = pd.concat([top_donor_dataframe, total_donation_list])
    
    # Add Indian currency
    top_donor_dataframe['Amount'] = top_donor_dataframe.apply(lambda row: locale.currency(row.Amount, grouping=True), axis=1)
    
    if type == "Individual":
        # Remove numbers from name
        top_donor_dataframe['Name'] = top_donor_dataframe.apply(lambda row: ''.join([i for i in row.Name if not i.isdigit()]), axis=1)
    
    pprint(top_donor_dataframe)
    
def send_email():    
    subject = f"Funds Raised in {current_quarter} - F.Y. {financial_year} | Raisers Edge"
    ...
    TEMPLATE = """
                ...
                <head> 
                  ...
                  <title>Funds Raised in Current Quarter | Raisers Edge</title><!--[if (mso 16)]>
                    <style type="text/css">
                ...
                <body style="width:100%;font-family:arial, 'helvetica neue', helvetica, sans-serif;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;padding:0;Margin:0"><span style="display:none !important;font-size:0px;line-height:0;color:#ffffff;visibility:hidden;opacity:0;height:0;width:0;mso-hide:all">Dear Team, Please find the below summary of Funds Raised in the {{fiscal_year}}.&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;‌&nbsp;</span> 
                              ...
                              <table width="100%" cellspacing="0" cellpadding="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                <tr> 
                                  ...
                                  <table width="100%" cellspacing="0" cellpadding="0" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                    <tr> 
                                      <td style="padding:20px;Margin:0"><p style="Margin:0;-webkit-text-size-adjust:none;-ms-text-size-adjust:none;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;line-height:21px;color:#333333;font-size:14px;text-align:justify">Dear Team,<br><br>Please find the below summary of Funds Raised in the {{current_quarter}} of F.Y. {{financial_year}}.</p></td> 
                                    </tr> 
                                  </table></td> 
                                </tr> 
                              </table></td> 
                            ...
                            <tr> 
                              ...
                              <table cellpadding="0" cellspacing="0" width="100%" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                <tr> 
                                  ...
                                  <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px"> 
                                    <tr> 
                                      <td align="center" style="padding:20px;Margin:0"><h1 style="Margin:0;line-height:36px;mso-line-height-rule:exactly;font-family:arial, 'helvetica neue', helvetica, sans-serif;font-size:30px;font-style:normal;font-weight:normal;color:#000000;text-align:center"><strong>Corporate</strong></h1></td> 
                                    </tr> 
                                    <tr> 
                                    ...
                                    <tr> 
                                      <td align="center" style="padding:20px;Margin:0">{{organisation_html}}</td> 
                                    </tr> 
                                    <tr> 
    ...
    emailbody = MIMEText(
        Environment().from_string(TEMPLATE).render(
            ...
            organisation_html = organisation_html,
            individual_html = individual_html
        ), "html"
    )
    
    message.attach(emailbody)
    attach_file_to_email(message, f'Donations_in_{current_quarter}_{financial_year}.xlsx')
    emailcontent = message.as_string()
...
try:
    ...
    # Get donation for current quarter
    start_date = current_quarter_start_date.strftime("%m-%d-%Y")
    ...
    # Merge donation dataframe with constituent data
    merge_dataframe(donation_dataframe, constituent_dataframe, 'value.constituent_id')
    
    # Merge donation dataframe with campaign list
    merge_dataframe(merged_dataframe, campaign_dataframe, 'campaign_id')
    
    # Get only required columns from merged dataframe
    quarterwise_donation_dataframe = merged_dataframe.filter(['constituent_name', 'value.constituent_id', 'value.date', 'value.id', 'amount.value', 'campaign_id', 'campaign_name', 'type'])
    pprint(quarterwise_donation_dataframe)
    
    # Get for Individual
    filter_dataframe(quarterwise_donation_dataframe, 'type', 'Individual')
    
    ## Save data to excel
    add_formula_to_columns(filtered_dataframe)
    write_to_excel(dataframe_excel, donations_workbook, 'Individual')
    
    ## Get summary for Individuals
    summarize_dataframe(dataframe_for_summary, 'Individual')
    
    ## Prepare HTML Table
    prepare_html_table(top_donor_dataframe)
    individual_html = html_output
    
    # Get for Organisation
    filter_dataframe(quarterwise_donation_dataframe, 'type', 'Organization')
    
    ## Save data to excel
    add_formula_to_columns(filtered_dataframe)
    write_to_excel(dataframe_excel, donations_workbook, 'Organization')
    
    ## Get summary for Organisations
    summarize_dataframe(dataframe_for_summary, 'Organization')
    
    ## Prepare HTML Table
    prepare_html_table(top_donor_dataframe)
    organisation_html = html_output
  ...
  subject = "Error while calculating funds raised in this quarter from Raisers Edge"
  print(subject)
  send_error_emails()
